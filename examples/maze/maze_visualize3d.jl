...
function rectangle(start, width, height, pos, cols, elms)
	...
	push!(pos, (start[1], start[2], 0.0))
	push!(pos, (start[1] + width, start[2], 0.0))
	push!(pos, (start[1], start[2] - height, 0.0))
	push!(pos, (start[1] + width, start[2] - height, 0.0))
	
	push!(pos, (start[1], start[2], 0.5))
	push!(pos, (start[1] + width, start[2], 0.5))
	push!(pos, (start[1], start[2] - height, 0.5))
	push!(pos, (start[1] + width, start[2] - height, 0.5))
	...
	#front
	push!(elms, (indx, indx+1, indx+4))
	push!(elms, (indx+1, indx+4, indx+5))

	#right	
	push!(elms, (indx+1, indx+3, indx+5))
	push!(elms, (indx+3, indx+5, indx+7))

	#back
	push!(elms, (indx+2, indx+3, indx+6))
	push!(elms, (indx+3, indx+6, indx+7))

	#left
	push!(elms, (indx, indx+2, indx+4))
	push!(elms, (indx+2, indx+4, indx+6))

	#top
	push!(elms, (indx+4, indx+5, indx+6))
	push!(elms, (indx+5, indx+6, indx+7))
	
	for i=1:8; push!(cols, (0.0, 0.0, 0.0)); end
end

function agent_model(start, width, height, pos, cols, elms)
	indx = length(pos)
	push!(pos, (start[1] + width*0.5, start[2] - height*0.25, 0.0))
	push!(pos, (start[1] + width*0.75, start[2] - height*0.75, 0.0))
	push!(pos, (start[1] + width*0.25, start[2] - height*0.75, 0.0))
...
function main()
	...
	#view = rotationmatrix_x(deg2rad(30)) * rotationmatrix_y(deg2rad(-30))
	view = lookat(Vec3((-1.2f0, -1.2f0, 3.6f0)), Vec3((0f0, 0f0, 0f0)), Vec3((0f0, 0f0, 1f0)))
	global a_model = foldp(*, rotate(0f0, Vec((0,0,1f0))), signal)
	a_view = translationmatrix(Vec{3, Float32}((-(wm / 2 - 0.5)*w, (hm/2 + 0.5 - env.start[1])*h , 0.0f0))) * scalematrix(Vec{3, Float32}((1.0/wm, 1.0/hm, 1.0f0))) * view
